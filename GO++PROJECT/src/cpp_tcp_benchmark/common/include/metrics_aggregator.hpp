...
public:
    MetricsAggregator(const std::string& protocol_name, std::size_t total_file_size_expected, std::size_t chunk_size_expected);
    ...
    void record_chunk_processed(std::size_t bytes_processed); // Legacy, might be replaced by RTT recording
    void record_chunk_verified(bool success); // Legacy, might be replaced by RTT recording


    void print_summary() const;
    void save_to_csv(const std::string& overall_metrics_file, const std::string& chunk_rtt_file) const;
...
private:
    ...
    std::size_t m_processed_chunks_count = 0; // Tracks how many chunks had RTT recorded
