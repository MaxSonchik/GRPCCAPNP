...
ChunkReader::ChunkReader(const std::string& filename, std::size_t chunk_size)
    : m_chunk_size(chunk_size), m_file_size(0), m_total_chunks(0), m_chunks_read_count(0), m_eof(false) {
    ...
    if (m_file_size == 0) {
        m_total_chunks = 0; // Or 1 if you consider an empty file as one "empty" chunk
        m_eof = true;
    } else {
        m_total_chunks = (m_file_size + m_chunk_size - 1) / m_chunk_size; // Ceiling division
    }
    // std::cout << "ChunkReader initialized for '" << filename << "'. File size: " << m_file_size
    //           << " bytes. Chunk size: " << m_chunk_size << " bytes. Total chunks: " << m_total_chunks << std::endl;
}
...
std::vector<char> ChunkReader::read_next_chunk() {
    ...
    buffer.resize(static_cast<std::size_t>(bytes_read)); // Resize to actual bytes read
    m_chunks_read_count++;
    return buffer;
}
...
std::size_t ChunkReader::total_chunks() const {
    return m_total_chunks;
}

std::size_t ChunkReader::file_size() const {
    return m_file_size;
}
std::size_t ChunkReader::chunks_read() const {
    return m_chunks_read_count;
}
