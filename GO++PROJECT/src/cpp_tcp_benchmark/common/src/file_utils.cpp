...
void generate_test_file_if_not_exists(const std::string& filename, std::size_t target_size) {
    ...
    if (!fs::exists(filename)) {
        std::cout << "Test file '" << filename << "' does not exist. Generating..." << std::endl;
        regenerate = true;
    } else {
        std::uintmax_t current_size = fs::file_size(filename);
        if (current_size != target_size) {
            std::cout << "Test file '" << filename << "' exists but has incorrect size ("
                      << current_size << " bytes vs expected " << target_size
                      << " bytes). Regenerating..." << std::endl;
            regenerate = true;
            fs::remove(filename); // Remove the old file
        } else {
            std::cout << "Test file '" << filename << "' already exists with correct size." << std::endl;
        }
    }
    ...
    if (regenerate) {
        ...
        std::cout << "Generating " << target_size / (1024.0 * 1024.0) << " MB file. This may take a while..." << std::endl;
        int progress_counter = 0;

        while (bytes_written < target_size) {
            std::size_t bytes_to_write = std::min(buffer_size, target_size - bytes_written);
            for (std::size_t i = 0; i < bytes_to_write; ++i) {
                buffer[i] = static_cast<char>(distrib(gen));
            }
            outfile.write(buffer.data(), bytes_to_write);
            if (!outfile) {
                 std::cerr << "Error writing to file " << filename << ". Disk full?" << std::endl;
                 outfile.close();
        ...
        std::cout << std::endl << "File '" << filename << "' generated successfully (" << bytes_written << " bytes)." << std::endl;
        outfile.close();
    }
}
